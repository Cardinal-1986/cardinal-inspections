...
| **CompanyCam import** | `COMPANYCAM_API_KEY` is set in Vercel. **The spec question is settled** — `references/companycam-api.md` now carries the read v1 `Photos → index` reference: `GET app.companycam.com/public_api/v1/photos`, cursor pagination, and server-side `start_date` / `end_date` / `tag_ids` / `project_ids` / `user_ids` filters | Six things are still unreadable and **must not be guessed** — the clipped tail of the `Photo` schema (**does v1 carry `description`?**), what `include` accepts, the date format (a bad one is a **422**, not a no-op), `include_total`, the OAuth scope names, and the rate limits. **`api/companycam-status.js` answers all six in one page load** — open `app.cardinalroster.com/api/companycam-status` after deploy and fill the reference in from what it says. The agent sandbox reaches no CompanyCam host, so that load is Theo's to do |
