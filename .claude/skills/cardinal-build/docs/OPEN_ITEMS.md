...
|---|---|---|
| **ABC Supply — wrong host + wrong paths on every data action** | ✅ **RESOLVED, 13 Aug — was never a 401, was never really just "fetch failed" either; the whole `api/abc.js` data layer was pointed at the wrong place.** The chain: 401-stale-note (12 Aug AM) → diagnostic fix shipped so the next failure would be self-explaining → live production retest returned `"Could not reach api.partners.abcsupply.com (ENOTFOUND: …)"` — a DNS failure, not a guess, because the auth host (a *different*, always-correct host) had already proven the credentials and network path were fine. Cross-checked against **9 of ABC's own endpoint reference pages** (`apidocs.abcsupply.com/get-branch/`, `/get-frequent-items/`, `/search-items/`, `/search-item-availability/`, `/price-items/`, `/get-recent-items/`, `/get-order-templates/`, `/search-branches/`, `/get-item-availability/`, `/place-orders/`, `/get-orders/` — 11 pages, all agreeing): the real hosts are **`partners.abcsupply.com`** (production) / **`partners-sb.abcsupply.com`** (sandbox, hyphenated suffix — not `sandbox.partners.*` like every other host in this file). **Separately**, 7 of the file's 9 data paths were also missing their real `/api/{family}/v{n}` prefix (only `frequents`/`recents` happened to already be right) — so a host-only fix would have traded one dead end for a wave of fresh 404s. `priceItems`'s request body was also wrong: real field names are `shipToNumber`/`branchNumber`/`purpose`/`lines[]` (not the invented `items[]`/`unitOfMeasure`/`variation`) — confirmed against ABC's own verbatim example JSON. All fixed, proven with a URL/body-capturing harness across all 9 actions (9/9 green) and negative-controlled against the pre-fix file (9/9 red — same file, same harness). | ✅ **RESOLVED end-to-end 13 Aug, build 763 — three stacked faults, each hidden behind the one in front.** (1) **Host did not exist** (`api.partners.abcsupply.com`, ENOTFOUND) → real hosts `partners.abcsupply.com` / `partners-sb.abcsupply.com`. (2) **`pageNumber` is REQUIRED on frequents/recents** — ABC's own docs call it *Optional*; production said otherwise verbatim: `Bad Request: Required query parameter pageNumber not specified`. **Trust the error, not the doc.** Only visible at all because the same session shipped `abcReason()` to stop discarding ABC's response body. (3) **Search and pricing were both speaking invented dialects.** Search sent `{query, branchNumbers, page, pageSize}` — *none of those fields exist*; ABC takes `filters:[{key,condition,values}]` + `pagination:{itemsPerPage,pageNumber}`, and the only free-text key is `itemDescription` (the availability-search endpoint accepts just itemNumber/branchNumber, so it could never serve the catalog box — the old availability-first ladder is deleted, not reordered). Pricing POSTed to the right path but the CLIENT read `j.items`/`j.prices` when ABC returns **`lines[]`** with **`unitPrice`** — so `arr` was always empty and every lookup fell through to "Branch will price — call them", a **wrong answer on a money surface**. Also: search rows carry `uoms:[{name,code}]` while frequents rows carry a flat `uom` string; only the flat one was handled. All request shapes now match ABC's published examples line for line (18/18 green, **9 red** on the pre-fix file). ⚠️ **Two deliberate non-decisions, recorded so they are not mistaken for oversights:** `embed:['branches']` carries per-branch availability rather than a `branchNumber` *filter* (a filter would silently drop everything that yard does not stock — a narrowing call to make against real data); and a single-token query that finds nothing is retried as `itemNumber equals` (Theo works from invoices full of codes like `02OCTDDML`), while a multi-word miss is not. | **Try it.** Search a description ("Duration shingles"), Frequent items, and Price at my branch. Remaining unknowns are now *data*, not shape: which account suffix ABC accepts (`2153354-2` from the portal vs `2153354-0001` from the invoice — both 400'd on the OLD code, neither has been tried since the pageNumber fix), and whether ship-to `0003` is the right per-job value. Any failure now names itself. **Branch 106 confirmed** from two ABC documents. |
