...
**Fail closed, and ACH is ACH (1199).** The pay link (`api/pay.js`) and the shared document's pay bar (`api/share.js`) compute what is owed from the collections ledger, and a **failed read of that ledger is never treated as an empty one**: any failed lookup throws, the pay route answers 503 with `Retry-After` and creates no checkout session, and the document shows a *Balance check unavailable* bar rather than silently dropping the pay bar. The resolver is byte-identical in both files and `gate_1199` asserts it. The webhook records the method the event implies — a cleared bank debit (`async_payment_succeeded`) is `method:'ach'`, a card completion `'card'` — where it wrote `'card'` for both since 1151. ⚠ Known and held: `jobFinance()` adds manual extras and honours legacy worksheet payments; the server resolver does neither (OPEN_ITEMS, 9 Sep 2026).
