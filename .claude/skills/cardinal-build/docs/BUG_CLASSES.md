...
| **a scroller that fits with ZERO slack** (984; latent long before it) | an `overflow-x:auto` strip whose content exactly equals its client box is not "fitting" — it is one character from hiding something, and `scrollbar-width:none` means nothing shows that it did. `.cr-cth-tabs` measured scrollWidth 354 == clientWidth 354 at one-digit counts, so the first real claim count pushed "Closed" off the edge silently. **Test with realistic data, not the fixture's zeroes — single digits were the one case the broken strip could still fit, so a fixture-only check could not have failed.** | ❌ **no mechanical check yet.** `sentinel` `OVERFLOW` watches the PAGE scrolling sideways and cannot see this, because the strip legitimately scrolls instead of breaking the page. The check would be: for every `overflow-x:auto` element, flag `scrollWidth > clientWidth` where the scrollbar is hidden. **There are 27 such scrollers in the app and this is the first one anyone measured** |
...
**Seen: build 1016, on all four contract templates at once, invisible since 747.**
...
read CSS, and do not trust a screenshot of page 1. `gate_1016.mjs`.
=======
