/*
 * Build 842 render proof — Crew Dispatch + Repairs band.
...
const TAG = FILE.includes('841') ? '841' : '842';  // 841 = repairs band ABSENT (neg control)
...
try {
  ...
  const hasBand = await page.evaluate(() => !!document.querySelector('#cr-disp .dband.repband'));
  if (TAG === '841') {
    ok('repairs band is ABSENT on build 841 (negative control)', !hasBand, hasBand);
