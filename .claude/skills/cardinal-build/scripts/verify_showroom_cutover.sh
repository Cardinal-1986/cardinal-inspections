...
get(){ curl -sS -m 90 -H 'Cache-Control: no-cache' "$1" -o "$2" -D "$2.hdr"; }
...
ok "the Cardinal CRM is no longer served here" \
   "$(grep -q 'Cardinal Client Resources' "$D/sr" || echo 1)" "$SR_B bytes"
...
check_dest(){ get "$1" "$D/d"; local t; t="$(title "$D/d")"
