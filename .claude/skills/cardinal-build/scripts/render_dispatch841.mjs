/*
 * Build 841 render proof — Crew Dispatch.
 ...
 *   node render_dispatch841.mjs                         # 841 -> GREEN
 *   node render_dispatch841.mjs /path/to/index_v840.html   # 840 -> RED (no CardinalDispatch)
 */
...
const TAG = FILE.includes('840') ? '840' : '841';
...
const SEED = {
  ...
  appointments: [], estimates: [], inspection_reports: [], punch_items: [], insurance_claims: [],
};
...
try {
  ...
  const present = await page.evaluate(() => typeof (window.CardinalDispatch && window.CardinalDispatch.open) === 'function');
  ok(`CardinalDispatch.open exists (${TAG === '841' ? 'expected' : 'should be ABSENT on 840'})`,
     TAG === '841' ? present : !present, present);
  if (TAG !== '841') {
    console.log(`\n${fail === 0 ? 'GREEN' : 'RED'} — ${pass} passed, ${fail} failed  (negative control)`);
    await browser.close(); process.exit(fail === 0 ? 0 : 1);
  }
