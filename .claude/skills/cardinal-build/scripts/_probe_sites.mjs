    ...
    try {
      res = await page.evaluate(`axe.run(document, {
        ...
        v: r.violations.map(x => ({
          ...
          all: x.nodes.map(nd => ({ t: String(nd.target[0]||'').slice(0,120), html: String(nd.html||'').slice(0,190) }))
        }))
      }))`);
    } catch (e) {
...
if (JSON_OUT) {
  for (const s of res.sites) {
    if (s.id !== 'label' && s.id !== 'select-name') continue;
    for (const nd of (s.all || [])) console.log(`[${s.id}] ${s.state}\n    ${nd.t}\n    ${nd.html}\n`);
  }
}
