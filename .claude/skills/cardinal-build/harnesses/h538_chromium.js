...
const now = fs.readFileSync(SC + '/app/index_v538.html', 'utf8');
const prev = fs.readFileSync(SC + '/app/index_v537.html', 'utf8');
...
(async () => {
  ...
  for (const [mode, want] of [['DARK', { cardinaltruth: 'rgb(239, 107, 107)',
    ...
    await p.screenshot({ path: SC + `/lnav538-${mode.toLowerCase()}.png`, fullPage: true });
  }
