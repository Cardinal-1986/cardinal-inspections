...
- **Scope is explicitly fenced, in both the module banner and the API header:** the Library files *reference* material — building code, roofing, siding, windows, gutters, manufacturer specs. It has **no knowledge of projects, clients, inspections, photos or Company Documents, and must never be pointed at them.** Respect that boundary; it is a stated design constraint, not an oversight.
