...
>
> ⚠️ **The character count moved again at 1182: 5,674,745 → 5,411,417** (−263,328 from the 1178
> figure; −278,531 from 1181, which is what 1182 actually removed). ⚠ The *deployed tree* only
> drops **169 KB, not 272** — a 103 KB file joined it. The real win is first load. The
> inspection-report cover logo was a 139,982-char base64 PNG written into the file **twice**,
> and it is now `cardinal-report-logo.png` at the repo root. **`*.html` at root is 20 still** —
> the PNG is an image, not a page. See the warning immediately below before removing another
> embedded asset.
>
> ### ⚠️ AN EMBEDDED ASSET IS NOT UNREFERENCED JUST BECAUSE NOTHING NAMES IT
>
> **Build 1182 deleted the duplicated logo blob and silently broke five things**, with every
> mechanical gate green. Nothing referenced the image by a name — **five separate places dug the
> data URI back out of a template constant by regex** (`.match(/class="…-logo" src="(data:image…`):
> `ESTIMATE_TEMPLATE`, `buildEstimate()`, the login/editor brand-logo IIFE (which did
> `if(!m) return;` and so **abandoned the whole function**, taking the daily login quote with it),
> and `cardinalLogo()` in `cr-epub-script`. Three substituted `''`; nothing threw. Every
> client-facing estimate and contract would have printed with no letterhead.
>
> **So before deleting any embedded asset, grep for a regex that reads it back out of its own
> host constant, not just for its name.** The path now lives once, in
> `var CARDINAL_LOGO_SRC = '/cardinal-report-logo.png';`, and `gate_1182` goes red if any
> extraction returns.
>
> ⚠️ **`gate_types`' globals generator had the same bare-regex flaw and this build tripped it:** a
> *comment* saying `window.Foo =` invented a global, and an invented global **masks the TS2304 the
> gate exists to catch**. It now runs through the lexer (measured: naive 193, lexer 193, zero
> ghosts — closed with no count moving).
>
...
| **`gate_types.py`, `gate_dupes.py`, `gate_a11y.mjs`, `gate_1206.mjs`** | **the FOUR RATCHETED standing gates — a baseline per code / name / rule, which may fall but never grow.** `gate_types` is tsc's TS2304 over the CONCATENATED blocks with a globals list generated from the file's own `window.X=` assignments — **it is this repo's cross-block `no-undef`, so do not build a second one**. `gate_dupes` watches a name gaining a definition. `gate_a11y` is axe-core over the sentinel's own state walk: names, roles, labels, structure — **contrast is deliberately OFF there**, because the sentinel's INK owns it and two instruments answering one question in different numbers is how a real failure gets argued about. Existing debt is baselined so it blocks nothing; a NEW violation is red the build it lands. ⚠️ **Run them EVERY build** — run at merge time on a twelve-build arc, every finding becomes archaeology (30 Aug). **`gate_1206` (10 Sep) is the fourth**: the 44px tap-target floor, measured across all 32 sentinel states at 390px. ⚠️ **It measures the HIT AREA, not the box** — `#cr-disp .job .mv` is a 15×15 element with a 45×45 `::after` pad (build 1040), and the audit that produced the gate recorded that pad as the app's worst tap target. Five selectors are asserted outright, 24 are baselined. `--list` prints the debt; `--selftest` poisons a control to 22px and requires the sweep to see it. **Not in `gate_chromium`** — that runner is for per-build gates with negative controls | tsc / Python / Chromium |
...
| `api/*.js` | **36** serverless functions (ESM) — re-counted 10 Sep 2026 at build 1212, which **retired `api/estimate-to-contract.js`**: no caller, and its `select` named `client_name` and `estimate`, columns `projects` has never had, so every call 404'd. The row said **29** and that was an @808 figure. The `→ Contract` button never used the route — `cr-e2c` builds the contract in the browser — and the `contracts` table is untouched |
| `sw.js` | service worker — push + offline shell |
| `manifest.json`, `icon-*.png`, `apple-touch-icon.png` | PWA assets |
| **`cardinal-report-logo.png`** | **the inspection-report cover logo (1182), 104,968 bytes.** Was a base64 data URI written into `index.html` **twice**. Referenced by both report templates, by `ESTIMATE_TEMPLATE`, by `buildEstimate()`, by `cardinalLogo()` and by the login/editor brand logo — all through the one constant `CARDINAL_LOGO_SRC`. **Deliberately ships**; it is fetched by client-facing documents opened via `location.origin` share links, so it must never be added to `.vercelignore` |
