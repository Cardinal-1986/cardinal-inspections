...
function prompt(changes) {
  ...
  return (
    ...
    '2. Change nothing that is not listed above. Surfaces not listed keep their ' +
    'current material and colour.\n' +
    ...
    '5. Return a single edited photograph.'
  );
}
...
async function askGeminiImage(model, key, image, mime, text, bare) {
  ...
  if (!bare) body.generationConfig = { responseModalities: ['IMAGE', 'TEXT'] };
    ...
    for (const model of IMAGE_MODELS) {
      for (const bare of [false, true]) {
        try {
          ...
          if (r.img) { result = r; via = model; break; }
          ...
          lastErr = e;
          if (e.status === 503 || e.status === 429) { await sleep(1200); continue; }
          if (e.status === 400 && !bare && /modalit|generation_?config/i.test(e.message || '')) continue;
        }
